"""
wrapper functions for using OpenAI's gym environment; pulled from Berkeley's deepRL HW3
slight modifications to wrap_deepmind to allow for downsampling of pixels
from environment: (84, 84) --> (42, 42)
"""
import gym
import cv2
import random
...
# def wrap_deepmind(env, downsample):
def wrap_deepmind(env, downsample=False):
    ...
    if 'FIRE' in env.unwrapped.get_action_meanings():
        env = FireResetEnv(env)
    # env = ProcessFrame84(env, downsample)
    if not downsample:
...
def get_env(env_id, seed, downsample=False):
    # env_id = task.env_id
